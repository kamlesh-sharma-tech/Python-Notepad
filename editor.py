...
format.add_command(label="Color Picker", image=font_color,compound=LEFT, command=lambda: color_picker())
format.add_command(label="Find/Replace",image=find_icon,compound=LEFT,command=lambda: find())
...
for i in color_theme_dict:
      color_theme.add_radiobutton(label=i,image=color_theme_icons[count],variable=theme_choice,compound=LEFT,command=lambda:change_theme())
      count+=1
...
italic_btn = ttk.Button(tool_bar, image=italic_icon,command=lambda: change_italic())
italic_btn.pack(side=LEFT, padx=15)
underline_btn = ttk.Button(tool_bar, image=underline_icon,command=lambda: change_underline())
underline_btn.pack(side=LEFT, padx=15)
align_left_btn = ttk.Button(tool_bar, image=align_left,command=lambda: left_align())
align_left_btn.pack(side=LEFT, padx=15)
align_center_btn = ttk.Button(tool_bar, image=align_center,command=lambda: center_align())
align_center_btn.pack(side=LEFT, padx=15)
align_right_btn = ttk.Button(tool_bar, image=align_right,command=lambda: right_align())
...
def save_as_file(event=None):
    ...
    try:
        ...
        if content > str(0):
            ...
            messagebox.showwarning("Can't Saved!","File is empty, need some content...")
    except:
        return
...
def find(event=None):
    ...
    replace_btn=ttk.Button(find_frame,text="Replace",command=lambda: replace())
    replace_btn.grid(row=2,column=1,padx=4,pady=4)

    exit_btn=ttk.Button(find_dialog,text="Quit Window",command=lambda: exit_find())
    exit_btn.pack()
...
def about_notepad():
    ...
    editor_icon_label=Label(about_dialog,image=editor_icon,bg="darkslategray")
    editor_icon_label.pack()

    sugg=Label(about_dialog,wraplength=400,font=("Arial",11),bg="darkslategray",fg="white",text="This is a simple text editor with all the basic functionalities that you need in a text editor like open,save,cut,copy etc.....")
    sugg.pack(pady=10)

    creator=Label(about_dialog,text="Notepad is created by Kamlesh Sharma",font=("Arial",16),fg="yellow",bg="darkslategray")
    creator.pack(pady=(30,10))

    exit_btn= ttk.Button(about_dialog,text="Quit Window",command=lambda: exit_about_window())
    exit_btn.pack()
...
def change_italic(event=None):
    ...
    if text_property.actual()["slant"] == "italic":
        editor.configure(font=(current_font_family,current_font_size,"roman"))
    editor.focus()
